...
export function CaseEditor({ value, onChange }: { value: CaseFormValue; onChange: (value: CaseFormValue) => void }) {
  ...
  return <Form layout="vertical" className="case-editor">
    ...
    <Form.Item label="arguments (JSON)"><JsonCodeEditor value={JSON.stringify(value.arguments ?? {}, null, 2)} height="180px" onChange={(source) => { try { onChange({ ...value, arguments: JSON.parse(source || "{}") }); } catch { /* keep editing */ } }} /></Form.Item>
    ...
    <Form.Item label={text("structuredEquals（JSON 部分匹配）", "structuredEquals (partial JSON match)")}><JsonCodeEditor value={assertions.structuredEquals === undefined ? "" : JSON.stringify(assertions.structuredEquals, null, 2)} height="140px" onChange={(source) => { if (!source.trim()) updateAssert({ structuredEquals: undefined }); else try { updateAssert({ structuredEquals: JSON.parse(source) }); } catch { /* keep editing */ } }} /></Form.Item>
  </Form>;
}
